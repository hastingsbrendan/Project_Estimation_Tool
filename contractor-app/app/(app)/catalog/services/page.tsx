...
export default async function ServicesCatalogPage() {
  ...
  // Fetch all items so the preset typeahead can still surface materials,
  // even though the table itself filters to services only.
      ...
      <div className="flex items-baseline justify-between">
        <div>
          ...
          <p className="text-sm text-foreground-muted mt-1">
            Labor / service items. Each row can be linked to a bundle of materials
            (presets) — when you add a service to a project, the bundled materials
            are offered for one-click insertion.
          </p>
        </div>
        {services.length > 0 && (
